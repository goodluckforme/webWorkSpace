databasepage = {
	...
	getFontSize: function() {
		var fontsize = this.getItem(this.cacheKey.fontSize) || {
			text: "中",
			value: "1.3em"
		};
		return fontsize;
	...
	_saveNewToDb: function(transition, data, categoryid, sortindex, onSuccess) {
		...
		transition.executeSql('select count(article_id) as num from article_list where article_id = ?', [data.article_id], function(tr, result) {
			...
			if(result.rows.item(0).num != 0) { //已存在记录，更新
				transition.executeSql('update article_list set article_content=?,flower_count=?,egg_count=?,collect_count=?,warn_count=?,comment_count=?,categoryid=? where article_id = ?', [data.article_content, data.flower_count, data.egg_count, data.collect_count, 0, data.comment_count, categoryid, data.article_id], function(tr, result) {
						onSuccess(result);
					},
					function(tx, error) {
	...
	*/
	...
	saveArticle: function(data, articleid) {
		var that = this,
			path = "all_list/" + (articleid || data.id) + ".json";
	...
	_downLoadFile: function(options) {
		var that = this,
			path = options.folderPath + options.dstFile,
			index = path.indexOf('?');
		if(index > 0)
